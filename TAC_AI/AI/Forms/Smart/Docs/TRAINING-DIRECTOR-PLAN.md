...
bounded verbs, raw telemetry). All four new daemons register via
...
`DaemonWatchdog.cs:30-41` — 9 entries currently; ScanAndRespawn iterates the array,
NOT the `_factories` dict at line 96-98). Without that array extension, registered
...
pattern (**lock-based** ring + sort-on-read for p50/p99/var — `PathRequestBackpressure.cs:152-178`
...
`LearningService.cs:78` + `SnapshotManager.cs:21,23,28` convention), NOT
...
Written to:
  ...
  * IMGUI panel via `DrawPathingDebugGUI` tail — **panel resized**: when
    ...
    `SmartForm.cs:643-645` expands to ~560×400 px with `GUI.BeginScrollView` +
...
### 7.1 Checkpoint triggers
  ...
  1. **AUTO-HEALTHY**: every Director tick where ALL constraints have been HEALTHY for
     ...
     **Piggy-back mechanism (MINOR-fix)**: Director subscribes to a NEW
     ...
     latest profile — NOT a separate write (no double-saving). **3-condition skip
     ...
     auto-checkpoint can capture mid-rollback torn state.
...
internal `_params.Length` (LoadParameters at OpponentIntentClassifier.cs:350-351 throws
...
LearningService.cs:227) defers until pause holder releases. **Trainer FSM edit
sites enumerated (BLOCKER-fix)**: (a) OnlineTrainer.cs:355 changes from
`if (_hostLost && _phase == Phase.Active)` to
`if ((_hostLost || Volatile.Read(ref _directorPaused)) && _phase == Phase.Active)`;
(b) line 391 changes from `if (!_hostLost)` to
`if (!_hostLost && !Volatile.Read(ref _directorPaused))`;
(c) line 407 unchanged (IsHost is orthogonal). Memory barrier `Volatile.Read(ref _directorPaused)`
immediately before the `_phase = Phase.Active` write at line 396 ensures ordering.
...
(2) The one policy-dependent model (`ActionValueEstimator`) has zero producers
in v0.2 — its queue is empty until v0.3 ships ActionValue producer
infrastructure, so contamination surface is nil in v1. (3) Queues are bounded
(`BoundedQueue` capacity 4096 per type-erased dictionary) — contamination is
small even when present. **Observability**: forced post-rollback digest emits
one extra `QUEUE-CARRYOVER:<n-events>` line per model so operator sees what
survived.
...
— bringing the canonical daemon roster from 13 (post-base-plan-extension state; current
source state is 9 verified at DaemonWatchdog.cs:30-41 + base plan adds TrainingDirector/DirectorInbox/ScenarioWorker/ChunkRegen
= 13) to **14**. The TrainingDirector does NOT call `BehaviorGuardWorker.Tick(deltaSec)`
...
Position / velocity / altitude inputs come from `BeliefSnapshot.ByTech` (the same pattern
§10 `AllyProtectionPublisher` uses).

**Rare main-thread-only reads** (terrain height, sea level, pitch, tank-state fields)
are marshaled via a **`GuardEvaluationRequest` round-trip protocol** (MAJOR-fix —
original "+30/+20 LOC" undercount):
  ...
  * **TankAIHelper field reads**: `Provoked` (int, line 3117), `FrustrationMeter` (int, line 321), `lastEnemyGet`, `AILimitSettings`, etc. require Volatile.Read snapshots — for the `FrustrationMeter` MAJOR thread-affinity concern, TankAIHelper gains a NEW `volatile int PublishedFrustrationMeter` companion written from UpdatePhysicsInfo on main thread, read by GuardWorker.
...
| **WaterloggedTech** | 40 s | observe | `AuthoredTerrain != Sea` **AND** `AIType ∉ {Buccaneer, Aviator, Astrotech}` **AND** `tank.boundsCentreWorld.y < KickStart.WaterHeight − 2.0 m` ≥ 75% ticks (BLOCKER-fix: `ManWorld.SeaHeight` was fictional; use `KickStart.WaterHeight` per Modified-tree convention at TerrainQuery.cs:26 — or alternatively `Singleton.Manager<ManGameMode>.inst.GetCurrentModeSeaLevel()`. Plan picks `KickStart.WaterHeight`. tank.boundsCentreWorld read is main-thread; marshaled via GuardEvaluationRequest per §9.2) **AND** mean(`recentSpeedXZ`) < 1.0 m/s **AND** net displacement < `0.5 m/s × windowSec` | Hunter, Gatherer, Patrol, RepairSupport (BLOCKER-fix: `Prospector` dropped — AIType not SmartIdentity) | any |
...
`altitude_above_terrain` is computed as `tank.boundsCentreWorld.y -
ManWorld.inst.TileManager.GetTerrainHeightAtPosition(pos, out bool onTile)`
...
`pitch_attitude` is computed as `Mathf.Asin(Vector3.Dot(tank.transform.forward, Vector3.up)) * Mathf.Rad2Deg`
(MAJOR-fix: pitch had no source primitive; bundled into the same `GuardEvaluationRequest`
envelope as altitude + sea level so a single envelope flush carries everything for all aircraft
guards). The query runs on main thread via the `GuardEvaluationRequest` queue (§9.2).
...
Tee points for `IdentityReplayBank`: at the **typed-queue enqueue sites** (corrected
locations: IntentEvent at `TargetObservationSequenceBuffer.cs:186` — NOT 114;
ResidualEvent at `LeadResidualRecorder.cs:99`; ThreatEvent at
`LearningService.cs:694`). **ActionValueEvent has NO producer site in v0.2**
(verified by grep — only ctor at ActionValueEstimator.cs:21 exists). Tee at 3 sites
not 4; `replay_bank(*, ActionValue)` no-ops until v0.3 ActionValue producer ships
(e.g. in ContinuousController after action selection — out of scope for Director v1).
...
| `Smart/Director/Publishers/BlockDeliveredPublisher.cs` | `ModuleItemHolder.TakeItemEvent` subscriber with carrier-holder attribution | 180 |
...
| `Smart/Learning/ProfilePersistence.cs` | `RestoreFromCheckpoint(modelName, slot)` routes existing `.previous`/`.penultimate` chain at `ProfilePersistence.cs:64-95`. `EnumerateExpiredPreSnapshots(maxAge)` for Director's pre-action cleanup. Reuses existing byte format. **B3 fix: `ProfilePersistence.Save`'s per-model loop at lines 105-130 MUST wrap `m.StoreParameters(weights)` in `lock(m.SaveMutex) { m.StoreParameters(weights); }`.** 4 locks per save, each microseconds. Required because `AutosaveWorker.RunLoop` calls `SaveProfile` from a worker thread while the trainer holds `SaveMutex` element-by-element during `_adam.Step(_params, grad)` (element-by-element write at `OnlineTrainer.cs:128-143`); without the wrap the persisted vector is torn-but-CRC-valid. Verified `ProfilePersistence.cs:105-109` is bare `Array.Copy` with no lock today | 60 |
| `Smart/Integration/SmartEventBridge.cs` | `InstallDirectorPublishers()`/`UninstallDirectorPublishers()`. One-line tee at line 273 KillScored publish to feed `IdentityReplayBank`. Per-minute `DamageObserved` `Interlocked.Increment` counter for `damage_events_per_min` constraint | 90 |
...
| `Smart/Threading/DaemonWatchdog.cs` | (Guard extension) Extend `CanonicalRoster` with `GuardWorker`. Update header docstring "13 long-running daemon label strings" → "14 long-running daemon label strings". **STRIKE the AbortSurvivalTests.cs update** — V-fix confirmed no hard-coded count assertion exists in the test file | 8 |
...
which surfaced additional undercounts in §12.2 modifications (V-fix MAJOR).
  ...
  * **New files**: ~6 440 (base) + ~2 980 (guard layer; was ~3 130 — ~150 LOC drop
    ...
    trimmed to ~200 LOC (was ~240) — RaiseBaseReturn surface dropped) + ~80 (NEW
    PathingService.cs wrapper — R3-B1: `IsReachable(origin, dest, capability)`
    ...
    "unknown / assume-reachable" until first refresh. Plus `LastQueryFailedRecently`
    per V-fix; was fictional `AIEPathQuery`. Do NOT call AIEPathMapper /
    AIEAutoPather2D/3D from background threads — non-thread-safe. **M1 fix:
    `PathingService.Init` at lines 142-143 MUST call
    `DaemonWatchdog.RegisterCanonical("ThreatFieldRebuild", ThreatFieldRebuildFactory)`
    and `DaemonWatchdog.RegisterCanonical("PathSolve", PathSolveFactory)` alongside
    the existing `EnqueueLongRunning` calls — pre-existing gap: those two daemon
    names are on the `DaemonWatchdog.cs:30-41 CanonicalRoster` but receive no
    factory in `PathingService.Init`, so the watchdog logs
    daemon-respawn-no-factory on death. Director plan fixes this as a parallel
    concern.**)
    + ~80 (NEW ProjectileFiredPublisher per V-fix; was fictional
    `FireControl.ProjectilesFiredCount`) = **~9 580** (was ~9 730).
  * **Modified files** (deltas): ~975 (base) + ~690 (guard) + V-fix adjustments:
    ...
    +15 DirectorState (60 from 45) = **~2 150** (was ~1 665).
  * **Grand total: ~11 730 LOC** (was ~11 880; OverdueDelivery telemetry-only
    reconciliation dropped ~150 LOC from the guard layer — 15s injection-delay
    timer, Q-margin override gate, BaseReturnGoalSource mailbox writer shrunk to
    telemetry observer, cooldown tracking, GuardInjectedGoals slot management
    all dropped). **52 new files** unchanged (BaseReturnGoalSource.cs renamed in
    place to BaseReturnTelemetryObserver.cs — same file slot, repurposed).
  ...
  * **Critical reservation**: many V-fix bumps are bookkeeping correctness;
    ...
    boilerplate was overestimated. Treat ~11 730 as upper bound.
  * **Test-only LOC (amendment A3 — separate from core v1 totals)**: ~400 LOC
    ...
    to the ~11 880 grand total above.

---

## 13. Key design decisions

  * **3 layers × 4 daemons** (`TrainingDirector`, `DirectorInbox`, `ScenarioWorker`,
    `ChunkRegen`). Director is a SINGLE 1 Hz daemon, not 4 per-model daemons. All
    register via `DaemonWatchdog.RegisterCanonical` + `WorkerHealthMonitor.RegisterCanonical`,
    mirroring `TrainerWorker`. Zero new lifecycle code; free auto-respawn.
  ...
  * **`actionvalue_loss_variance_ratio` uses `LossBefore` and reward-magnitude normalises.**
    ...
    so HQ-destruction reward spikes don't false-trip.
  ...
  * **Director shutdown plug is in `SmartRuntime.Shutdown`** (line ~931, BEFORE
  ...
  * **DaemonWatchdog header docstring updated** from "9 long-running daemon label
    strings" to "13 …" — this is a verifier-relied-on assertion.
...
### 14.1 Operator open questions
  ...
  * **MP soft-correct safety.** Largely moot under V-fix telemetry-only
    ...
    `IdentityOutcome` publishers, already host-only. Re-verify if guard is
    ever promoted beyond telemetry-only.
  ...
  * **Guard re-arm chronic conditions.** Re-fire requires the pathology test to
    pass again after cooldown. For chronic conditions (Gatherer that simply
    cannot reach base due to terrain), `OverdueDelivery` would fire-cool-fire-cool
    indefinitely. Should there be a per-tech tries-exhausted state escalating to a
    one-shot operator-surface message after 5 cycles? Plan doesn't specify.
  ...
  * **Live-build bugs (patch independent of Director timing)** (amendment A6).
    Two amendments from the decompile-deep-dive identified bugs that affect the
    shipped Smart build at `eac2e0e` and should be triaged for the next live
    patch independent of the Director branch:
    * **M1**: `PathingService.Init` at lines 142-143 enqueues
      `ThreatFieldRebuild` + `PathSolve` but never calls
      `DaemonWatchdog.RegisterCanonical` for them despite their being on the
      canonical roster (`DaemonWatchdog.cs:30-41`). If either daemon dies the
      watchdog logs `daemon-respawn-no-factory-…` and they stay dead. Fix: add
      the two RegisterCanonical calls. ~5 LOC.
    * **B3**: `ProfilePersistence.Save` at lines 105-130 iterates models calling
      `StoreParameters` without `lock(m.SaveMutex)`. Concurrent `AdamState.Step`
      element-by-element write tears `Array.Copy`, producing torn-but-CRC-valid
      parameter vectors on save. May contribute to overnight training drift /
      NaN-cascade observed prior. Fix: wrap `StoreParameters` in
      `lock(m.SaveMutex)` (4 locks per save). ~5 LOC.

### 14.2 Verifier-pass appendix — disposition of reviewer findings

Legend: APPLIED = absorbed into this plan; DEFERRED = parked with named blocker;
SKIPPED = explicitly rejected with rationale.
...
scope and rationale; SKIPPED = explicitly rejected with rationale; DEFERRED =
parked.
...
| **Entropy push at unwired Evaluate** (MAJOR) | Pinned to `TrainOneMinibatch_FullBptt` at line 256 where softmax probs already computed. NaN guard `p[i] <= 1e-9f` → 0. | 1 | §4, §12.2 OpponentIntentClassifier |
...
| **Type-erasure for IdentityReplayBank** (MAJOR) | 4 type-specialized dictionaries (no boxing); `EventEnvelope<T>` wrapper with Source byte + ReplayCount; ActionValueEvent producer noted as v0.3-dependency (no production site in v0.2). | 1 | §5, §10, §12.1 IdentityReplayBank |
...
| RM-4 | Tank.AttachEvent / DetachEvent firing order vs engine block-count update | Reviewer-12 noted unverified order — HP cache invalidation timing matters; spawn-test will confirm. |
...
| RM-7 | TacticalGoal.AtCurrent as "neutral" sentinel kinematic equality threshold | §9.7 step 5 telemetry-only resolution sidesteps the original sentinel requirement; verify kinematic detector accuracy. |
| RM-8 | Director.Init full sequence vs ScenarioWorker / GuardWorker registration order | Spec'd in §2 Rationale but not yet exercised under abort-during-init races. |
| RM-9 | Stale guard mailbox slots vs Tank-pool recycle | Per-tech array cleared on OnTechDespawn; verify no slot leak across Tank pool reuse. |
| RM-10 | TrainerStats reservoir lock contention at 30 Hz × 4 models | Mirrors PathRequestBackpressure lock pattern (verified lock-based, not lock-free); cap not yet measured. |
...
| RM-12 | `Tank.Holders.ItemPickupEvent` cross-tank-only gating (Visible.cs:910,928) interaction with same-tank holder-to-holder moves | Conveyor sort within one tank does not fire — confirmed semantically but not under stress. |
...
| M1 | APPLIED-AS-IS | PathingService.Init at lines 142-143 MUST call `DaemonWatchdog.RegisterCanonical("ThreatFieldRebuild", …)` + `("PathSolve", …)` — pre-existing gap (names on CanonicalRoster but no factory registered). |
...
| B3 | APPLIED-AS-IS | ProfilePersistence.Save MUST wrap m.StoreParameters in `lock(m.SaveMutex)` (4 locks/save, microseconds) to prevent torn-but-CRC-valid persisted vector vs AdamState.Step element-by-element write. |
...
| A1 | APPLIED-AS-IS (doc-only) | Rollback EventQueue KEEP semantics: pre-rollback events RETAINED through rollback (3 of 4 models observation-based, ActionValue has zero v0.2 producers, queues bounded). §7.3 step 4.5 added with `QUEUE-CARRYOVER:<n-events>` forced-digest observability. No LOC impact. |
...
| A6 | APPLIED-AS-IS (live-build patch, separate from Director) | Two pre-existing bugs in shipped Smart build at `eac2e0e`, NOT Director-blocked, added to §14 backlog for the next live patch independent of Director timing: **M1** = `PathingService.Init` missing `RegisterCanonical` for `ThreatFieldRebuild` + `PathSolve` (~5 LOC); **B3** = `ProfilePersistence.Save` missing `lock(m.SaveMutex)` around `StoreParameters` (~5 LOC, may contribute to overnight drift / NaN-cascade). Both already noted inline in §12.3 (M1) and §12.2 ProfilePersistence row (B3); §14.1 backlog entry added so they get triaged for live release independent of Director branch. |
