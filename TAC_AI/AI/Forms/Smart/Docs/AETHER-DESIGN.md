...
- **No phantom marshalling claims.** Verification:
  ...
  - ~~`TeamBelief.InjectObservations`~~ — **REV 7 P5 Item 22:** TeamBelief.cs DELETED (orphaned class, zero `new TeamBelief(` callers verified by 10/10 reviewers). LOS aggregation now lives inline in `TeamRuntime.BuildTeamSnapshot` at `SmartRuntime.cs:401-429`; data source is `LineOfSightProducer.SnapshotForTeam` (main-thread producer) gated on `LineOfSightProducer.Enabled` (default OFF preserves v0.1 NeedsScout TRUE-always via the REV 3 S1 per-tech empty sentinel).
...
| ~~`AI\Forms\Smart\Coordination\TeamBelief.cs`~~ | **REV 7 P5 Item 22: DELETED** | Class was orphaned (zero `new TeamBelief(` callers tree-wide). LOS aggregation inlined in `TeamRuntime.BuildTeamSnapshot` (`SmartRuntime.cs:401-429`); data source is the new `LineOfSightProducer` (P5 Item 23). |
...
4. **Rewrite `BeliefState` class body + delete `KalmanUpdate` + delete `BeliefDecay`.** Rename `PerceptionWorker` → `AetherFuser` and replace its `Tick` body with the new drain/coast/publish loop. Inside `WorldModel.RecordObservation`: thin pass-through to `Observer.Submit`. `WorldModel.UpdateTeam`: call the new `BeliefStateFactory.WithTeam(prior, newTeam)` instead of `prior.WithTeam(newTeam)` (semantically identical). **REV 7 P5 Item 22 supersede:** the original migration step stubbed `TeamBelief.InjectObservations` to throw; v0.2 deletes `TeamBelief.cs` entirely as orphaned and inlines LOS aggregation in `TeamRuntime.BuildTeamSnapshot`. Build. The `SmartTestSuite` Kalman/BeliefDecay tests fail to compile (expected — they reference deleted symbols).
