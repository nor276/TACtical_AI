...
namespace TAC_AI.AI.Movement.AICores
{
    ...
    internal class SeaAICore : IMovementAICore
    {
        ...
        public void Initiate(Tank tank, IMovementAIController controller)
        {
            ...
            if (controller.Helper.Allied && controller.Helper.AutoAnchor)
            {
                if (tank.IsAnchored && !controller.Helper.PlayerAllowAutoAnchoring)
                    DebugTAC_AI.LogWarnFileOnly("SeaAICore-anchored-" + tank.name,
                        "[AIWARN] SeaAICore - Should NOT be active when anchored UNLESS we have autoAnchor! StaticAICore should be in control!");
            }
            else if (tank.IsAnchored)
            {
                DebugTAC_AI.LogWarnFileOnly("SeaAICore-anchored-NPT-" + tank.name,
                    "[AIWARN] SeaAICore - Should NOT be active when anchored UNLESS we have autoAnchor! StaticAICore should be in control!");
            }
        }
        ...
        public bool DriveDirectorRTS(ref EControlCoreSet core)
        {
            ...
            {
                if (helper.Attempt3DNavi && !(helper.FullMelee && helper.lastEnemyGet.IsLiveTechTarget()))
        ...
        public bool DriveMaintainer(TankAIHelper helper, Tank tank, ref EControlCoreSet core)
        {
            ...
            if (helper.AdviseAwayCore)
            {   //Move from target
                if (helper.lastEnemyGet.IsLiveTechTarget() && AIEPathing.IsUnderMaxAltPlayer(tank.boundsCentreWorldNoCheck.y))
                {
                    ...
                    if (helper.AIAlign == AIAlignment.Player && helper.lastPlayer.IsLiveTechTarget())
                    {
            ...
            {
                if (helper.lastEnemyGet.IsLiveTechTarget() && !helper.IsMultiTech && AIEPathing.IsUnderMaxAltPlayer(tank.boundsCentreWorldNoCheck.y))
